...
for i in $(seq $start 4); do
    ...
    # Run benchmark
    script -e -q -c "numactl -r all -i all /usr/bin/time --verbose -- ../bench_canneal_mt 64 50000 2000 ../../../datasets/canneal_35gb_int 400" output_r_i_${i}.txt
    [[ $? -eq 130 ]] && { echo "Interrupted. Exiting..."; exit 1; }
    
    # Save history with suffix
    cat /proc/mitosis/history > history_r_i_${i}.txt
done
